 ...
 */
public class SimpleListModelIgnoreCase extends AbstractListModel
implements ListModelExt, ListSubModel, java.io.Serializable {
	 ...
	 * @since 2.4.1
	 */
	public SimpleListModelIgnoreCase(Object[] data, boolean live) {
		if (data == null)
	 ...
	 */
	public SimpleListModelIgnoreCase(Object[] data) {
		this(data, false);
	}
	 ...
	 * @since 2.4.1
	 */
	public SimpleListModelIgnoreCase(List data) {
	...
	//whkuo, added two tolower
	public ListModel getSubModel(Object value, int nRows) {
		final String idx = value == null ? "" : objectToString(value).toLowerCase();
		if (nRows < 0) nRows = 10;
		final LinkedList data = new LinkedList();
		for (int i = 0; i < _data.length; i++) {
			if (idx.equals("") || _data[i].toString().toLowerCase().startsWith(idx)) {
				data.add(_data[i]);
				if (--nRows <= 0) break; //done
			}
		}
		return new SimpleListModelIgnoreCase(data);
	}
