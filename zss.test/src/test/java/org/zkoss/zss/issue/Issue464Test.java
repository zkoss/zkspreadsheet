...
//		System.out.println("cell style number before "+book.getInternalBook().getNumCellStyles());
		Range r = Ranges.range(sheet,0,0,row,column);
		CellOperationUtil.applyFillColor(r,"#ff0000");
		for(int i=0;i<=row;i++){
			for(int j=0;j<=column;j++){
				r = Ranges.range(sheet,i,j);
				CellStyle style = r.getCellStyle();
				String location = "At "+Ranges.getCellRefString(i, j) +" Row:"+i+", Column:"+j;
				Assert.assertEquals(location, "#ff0000", style.getFillColor().getHtmlColor());
			}
		}
		r = Ranges.range(sheet,1,1,row-1,column-1);
		CellOperationUtil.applyFillColor(r,"#ffff00");
		for(int i=0;i<=row;i++){
			for(int j=0;j<=column;j++){
				r = Ranges.range(sheet,i,j);
				CellStyle style = r.getCellStyle();
				...
				if(i>0 && j>0 && i< row && j<column){
					Assert.assertEquals(location, "#ffff00", style.getFillColor().getHtmlColor());
				}else{
					Assert.assertEquals(location, "#ff0000", style.getFillColor().getHtmlColor());
				}
			}
...
//		System.out.println("cell style number before "+book.getInternalBook().getNumCellStyles());
		Range r = Ranges.range(sheet,0,0,row,column);
		CellOperationUtil.applyFontHeightPoints(r, 20);
		for(int i=0;i<=row;i++){
			for(int j=0;j<=column;j++){
				r = Ranges.range(sheet,i,j);
				CellStyle style = r.getCellStyle();
				String location = "At "+Ranges.getCellRefString(i, j) +" Row:"+i+", Column:"+j;
				Assert.assertEquals(location, 20, style.getFont().getFontHeightInPoint());
			}
		}
		r = Ranges.range(sheet,1,1,row-1,column-1);
		CellOperationUtil.applyFontHeightPoints(r, 40);
		for(int i=0;i<=row;i++){
			for(int j=0;j<=column;j++){
				r = Ranges.range(sheet,i,j);
				CellStyle style = r.getCellStyle();
				String location = "At "+Ranges.getCellRefString(i, j) +"(Row:"+i+", Column:"+j+")";
				if(i>0 && j>0 && i< row && j<column){
					Assert.assertEquals(location, 40, style.getFont().getFontHeightInPoint());
				}else{
					Assert.assertEquals(location, 20, style.getFont().getFontHeightInPoint());
				}
			}
