...
public class Issue947Test {
	...
	public void testExportXlsBookWithPrintArea() throws IOException {
		...
		try  {
			Book book = Util.loadBook(this, "book/947-print-area.xls");
			Assert.assertTrue("No Exception", true);
			Exporters.getExporter("xls").export(book, os);
//			Filedownload.save(os.toByteArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "scale0.xlsx"); 
		} catch (Exception e) {
			Assert.assertTrue("Exception when load \"issue/book/947-print-area.xls\":\n" + e, false);
		} finally {
			 os.close();
		}
	}
	...
	@Test
	public void testExportXlsxBookWithPrintArea() throws IOException {
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		try  {
			Book book = Util.loadBook(this, "book/947-print-area.xls");
			Assert.assertTrue("No Exception", true);
