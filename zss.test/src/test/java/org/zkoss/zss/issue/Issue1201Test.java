...
public class Issue1201Test {
	 ...
	 */
	@Test
	public void testCreateSheetNPE() throws IOException {
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		try  {
			Book book = Util.loadBook(this, "book/blank.xlsx");
			Ranges.range(book).createSheet("temp");
			Assert.assertTrue("No Exception", true);
		} catch (Exception e) {
			e.printStackTrace();
			Assert.assertTrue("Exception when Ranges.range(book).createSheet(\"temp\"):\n" + e, false);
		} finally {
			 os.close();
		}
	}
