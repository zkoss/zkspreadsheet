...
zss.ActiveRange = zk.$extends(zk.Object, {
	...
	//prune: false
	/**
	 * Top frozen data, zss.FreezeActiveRange
	 */
	//topFrozen
	/**
	 * Left frozen data
	 */
	//leftFrozen, zss.FreezeActiveRange
	/**
	 * Current cached range. left, right:row index. top, bottom: column index
	 */
	rect: null,
	$init: function (data) {
		this.rows = {};
	...
	updateBoundary: function (dir, top, left, btm, right) {
		...
		var newRange;
		if (!rect) {
			this.rect = zss.ActiveRange.newRect(top, left, btm, right);
			return;
		}
			...
			case 'west':
				if(top == rect.top && btm == rect.bottom) { // normal case
					...
					if(top > rect.bottom || btm < rect.top) { // fetched block is totally outside of cached range
						newRange = zss.ActiveRange.newRect(top, left, btm, right);
	...
	insertNewColumn_: function (colIdx, size, headers) {
		...
		zss.ActiveRange.updateHeaders(this.columnHeaders, headers);
		var rows = this.rows;
		var rCol = rng.right;
	...
	insertNewRow_: function (rowIdx, size, headers) {
		...
		zss.ActiveRange.updateHeaders(this.rowHeaders, headers);
		var rows = this.rows;
		var lCol = rng.left;
		var rCol = rng.right;
		var r = rowIdx;
	...
	updateCells: function (v, dir) {
		...
		for (; i <= btm; i++) {
			var row = rows[i];
			if (!row) {
				row = rows[i] = zss.ActiveRange.newRow(src[s++], attrType, left, right, textAggregation, styleAggregation, mergeAggregation);
				//row contains wrap cell may have height Id on client side
				if ('jump' == dir) {
					oldRow = oldRows[i];
		...
		if (rowHeaderObj) {
			zss.ActiveRange.updateHeaders(this.rowHeaders, rowHeaderObj);
			//row contains wrap cell may have height Id on client side
			if ('jump' == dir) {
		...
		if (colHeaderObj) {
			zss.ActiveRange.updateHeaders(this.columnHeaders, colHeaderObj);
		}
	...
	newRect: function(tRow, lCol, bRow, rCol) {
        return {
            top: tRow,
            left: lCol,
            bottom: bRow,
	...
	updateHeaders: function(dest, src) {
		var headers = src.hs,
			i = src.s,
			end = src.e,
			j = 0;
	 ...
	 */
	newRow: function(v, type, left, right, texts, styles, mergeInfos) {
		var row = {
			r: v.r,
			heightId: v.h,
			cells: {},
			...
			update: function (attr, type, left, right, texts, styles, mergeInfos) {
				...
				for (; i <= right; i++) {
					var c = cs[i];
					if (!c) {
						c = cs[i] = zss.ActiveRange.newCell(src[j++], type, hId, texts, styles, mergeInfos);
						c.r = r;
						c.c = i;
					} else {
     ...
     * @param Object mergeInfos the merge info aggregation
     */
    newCell:function(v, type, heightId, texts, styles, mergeInfos) {
        var c = {
            /**
             * Row number
             */
            ...
            update: function (v, type, heightId, texts, styles, mergeInfos) {
                ...
                if (upMerge) {
                    ...
                    if (idx != undefined) {
                        ...
                        this.mergeCls = css;
                        if (this.mergeId) {
                            this.merge = zss.ActiveRange.newRect(info.t, info.l, info.b, info.r);
                        }
