...
public class ChartDataUtil {
	...
	public static ChartData getChartData(Sheet sheet, Rect selection,
			Type type) {
		org.zkoss.poi.ss.usermodel.charts.ChartData data = newChartData(sheet, selection, type);
		return toChartData(data);
	}
	
	public static ChartData toChartData(org.zkoss.poi.ss.usermodel.charts.ChartData data){
		return new ChartDataImpl(new SimpleRef<org.zkoss.poi.ss.usermodel.charts.ChartData>(data));
	}

	// following code refers from SSWorkbookCtrl
	private static org.zkoss.poi.ss.usermodel.charts.ChartData newChartData(Sheet sheet, Rect selection,
			Type type) {
		org.zkoss.poi.ss.usermodel.charts.ChartData data = null;
	...
	private static org.zkoss.poi.ss.usermodel.charts.ChartData fillXYData(XYData data,Sheet sheet, Rect selection) {
...
//				String endCell = spreadsheet.getColumntitle(rCol) + spreadsheet.getRowtitle(selection.getBottom());
				...
				horValues = DataSources.fromNumericCellRange(((SheetImpl)sheet).getNative(), cra);
			}
			//find values
//			int i = 1;
...
//				String endCell = spreadsheet.getColumntitle(c) + spreadsheet.getRowtitle(selection.getBottom());
				...
				values.add(DataSources.fromNumericCellRange(toPoiSheet(sheet), cra));
			}
		} else {
...
//				String endCell = spreadsheet.getColumntitle(selection.getRight()) + spreadsheet.getRowtitle(tRow);
				...
				horValues = DataSources.fromNumericCellRange(toPoiSheet(sheet), cra);
			}
			//find values
//			int i = 1;
...
//				String endCell = spreadsheet.getColumntitle(col)+ spreadsheet.getRowtitle(selection.getBottom());
				...
				cats = DataSources.fromStringCellRange(toPoiSheet(sheet), cra);
			}
...
//				String endCell = spreadsheet.getColumntitle(c)+ spreadsheet.getRowtitle(selection.getBottom());
				CellRangeAddress cra = toCellRangeAddress(rowIdx,c,selection.getBottom(),c);
				ChartDataSource<Number> val = DataSources.fromNumericCellRange(toPoiSheet(sheet),cra);
				vals.add(val);
			}
...
//				String endCell = spreadsheet.getColumntitle(selection.getRight()) + spreadsheet.getRowtitle(r);
	...
	private static Rect getChartDataRange(Sheet sheet,Rect selection) {
