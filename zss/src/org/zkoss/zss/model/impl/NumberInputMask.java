...
public class NumberInputMask {
	...
	public Object[] parseNumberInput(String txt, Locale locale) {
		...
		} else if (percent) { //percent => allow digit, decimal, paren
			if (Character.isDigit(c)) {
				sb.append(c);
			} else if (c == dot) {
				sb.append('.');
		...
		try {
			...
			return format == null ? 
					new Object[] {new Integer(Cell.CELL_TYPE_NUMERIC), val} :
					new Object[] {new Integer(Cell.CELL_TYPE_NUMERIC), val, format}; //double
